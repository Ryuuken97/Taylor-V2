...
const handler = async (m, {
  ...
  try {
    ...
    const stickerBuffer = await sticker4(buffer, !1, senderName, m.sender.split("@")[0]);
...
async function Quotly(data) {
  try {
    ...
    try {
      const fallbackResponse = await axios.get(`https://widipe.com/quotely?avatar=${encodeURIComponent(data.messages[0]?.from.photo.url)}&name=${data.messages[0]?.from.name}&text=${data.messages[0]?.text}`, {
