...
const handler = async (m, {
  ...
  try {
    if (command === "otasearch") {
      ...
      const otaku = conn.Button.setBody("Pilih anime di bawah ini.").setImage(anu[0].thumbnail).addSelection("Klik di sini").makeSections("Otakudesu", "rekomendasi");
      for (const v of anu) {
...
Sinopsis:
${anu.sinopsis}`;
      const otaku = conn.Button.setBody(teks).setImage(anu.image).addSelection("Klik di sini").makeSections("Otakudesu", "");
