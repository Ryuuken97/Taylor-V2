    ...
    const isButton = {
      start: async () => {
        ...
        if (!args[1] || !difficult.includes(args[1])) {
          return conn.Button.setBody("Select The Difficulty!").addSelection("Click Here!").makeSections("Difficulty").makeRow("", "Easy", "Select Easy", ".buckshot start easy").makeRow("", "Normal", "Select Normal", ".buckshot start normal").makeRow("", "Hard", "Select Hard", ".buckshot start hard").run(m.chat, conn, m);
        }
